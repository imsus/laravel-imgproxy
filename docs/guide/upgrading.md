...
description: Migrate v1 call sites to the 2.0.0 API.
---

# Upgrading from 1.x

v1 is still published on Packagist (`composer require imsus/laravel-imgproxy:^1.0`). Its source lives on the [`1.x` branch](https://github.com/imsus/laravel-imgproxy/tree/1.x), and the [v1 documentation is frozen](/1.x/guide/getting-started).
...
- 100% Pest type coverage and golden-vector signing tests, plus live checks against a real imgproxy in Docker (gated behind env vars; see [Testing Against a Real imgproxy](https://github.com/imsus/laravel-imgproxy/blob/main/README.md#testing-against-a-real-imgproxy)).

See [CHANGELOG](https://github.com/imsus/laravel-imgproxy/blob/main/CHANGELOG.md) for the full list.
