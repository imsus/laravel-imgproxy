...
| --- | --- | --- |
| `url` | `url(string $source, ?string $instance = null): Builder` | Build a URL for the given source on the default (or named) instance. |
...
Imgproxy::url('https://...');            // build on the default instance
Imgproxy::url('https://...', 'staging'); // build on a named instance
```
...
| `encoding` | `encoding(string $encoding): self` | — | Return a copy with a different source encoding (`base64` or `plain`). |
...
| `size` | `size(?int $width = null, ?int $height = null, bool $enlarge = false, bool $extend = false, Gravity\|string\|null $gravity = null): self` | `s:` | Set width, height, enlarge, extend, and gravity in one option. |
| `resizingType` | `resizingType(ResizeType\|string $type): self` | `rt:` | Set the resizing type without a size. |
...
| `trim` | `trim(float $threshold, ?string $color = null, bool $equalHor = false, bool $equalVer = false): self` | `t:` | Remove the surrounding background. The color is 3 or 6 digit hex. |
...
| `rawResponse` | `rawResponse(bool $enabled = true): self` | `raw:` | Respond with the raw unprocessed source image. |
...
| `autoRotate` | `autoRotate(bool $autoRotate): self` | `ar:` | Automatically rotate based on the EXIF orientation. |
| `flip` | `flip(bool $horizontal = false, bool $vertical = false): self` | `fl:` | Flip the image along the horizontal and/or vertical axes. |
| `enlarge` | `enlarge(bool $enlarge): self` | `el:` | Enlarge the image when it is smaller than the given size. |
| `extend` | `extend(bool $extend, Gravity\|string\|null $gravity = null): self` | `ex:` | Extend the image when it is smaller than the given size. Smart gravity not supported. |
| `extendAspectRatio` | `extendAspectRatio(bool $extend, Gravity\|string\|null $gravity = null): self` | `exar:` | Extend the image to the requested aspect ratio. Smart gravity not supported. |
...
| Method | Signature | imgproxy segment | Description |
| --- | --- | --- | --- |
| `stripMetadata` | `stripMetadata(bool $strip): self` | `sm:` | Strip the output image metadata (EXIF, IPTC, etc.). |
| `keepCopyright` | `keepCopyright(bool $keep): self` | `kcr:` | Keep the copyright info while stripping metadata. |
| `stripColorProfile` | `stripColorProfile(bool $strip): self` | `scp:` | Transform the embedded color profile to sRGB and remove it. |
| `preserveHdr` | `preserveHdr(bool $enable): self` | `ph:` | Keep high bit images high bit instead of downscaling to 8 bit. |
| `enforceThumbnail` | `enforceThumbnail(bool $enforce): self` | `eth:` | Always use the embedded thumbnail of the source image when available. |
| `returnAttachment` | `returnAttachment(bool $attachment): self` | `att:` | Return the processed image as an attachment instead of inline. |
...
| `imgproxyPreset` | `imgproxyPreset(string $name, string ...$more): self` | `pr:` | Apply server-side presets configured on the imgproxy instance. The server must have the preset registered. |

### Presets

| Method | Signature | imgproxy segment | Description |
| --- | --- | --- | --- |
| `preset` | `preset(string $name): self` | — | Compose a named preset from config onto the builder. Options are appended before any options chained after this call, so per-URL overrides win. |

### Security

| Method | Signature | imgproxy segment | Description |
| --- | --- | --- | --- |
| `maxSrcResolution` | `maxSrcResolution(int\|float $megapixels): self` | `msr:` | Redefine the maximum source image resolution in megapixels. Requires security options on the server. |
| `maxSrcFileSize` | `maxSrcFileSize(int $bytes): self` | `msfs:` | Redefine the maximum source image file size in bytes. |
...
| Method | Signature | imgproxy segment | Description |
| --- | --- | --- | --- |
| `raw` | `raw(string $segment): self` | *(verbatim)* | Append a processing option segment verbatim, without validation. Use for imgproxy options not yet covered by a typed method. |
...
| --- | --- | --- |
| `ResizeType` | `Imsus\LaravelImgproxy\Enums\ResizeType` | `resize()`, `resizingType()`, preset `resize` |
| `Gravity` | `Imsus\LaravelImgproxy\Enums\Gravity` | `gravity()`, `crop()`, `size()`, `extend()`, `extendAspectRatio()` |
