...
- Disk + path sources resolve through the shared `DiskUrl` resolver (`imgproxy()->image(DiskUrl::resolve(Storage::disk(...), $path))`), the same code path as the Storage macro and `Builder::disk()`, so the component URLs match the builder byte for byte. Public disks yield `url()`, private disks pre-signed `temporaryUrl()`.
