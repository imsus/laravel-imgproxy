...
- All 29 remaining free v4 options are typed on the builder: `resizeWithGravity`, `minWidth`, `minHeight`, `zoom`, `extendAspectRatio`, `focusPoint`, `trim`, `padding`, `autoRotate`, `flip`, `pixelate`, `stripMetadata`, `keepCopyright`, `stripColorProfile`, `preserveHDR`, `enforceThumbnail`, `formatQuality`, `skipProcessing`, `raw`, `cacheBuster`, `expires`, `filename`, `returnAttachment`, `preset`, `maxSourceResolution`, `maxSourceFileSize`, `maxAnimationFrames`, `maxAnimationFrameResolution`, `maxResultDimension`. The standalone `resizingType` is not typed — the type is set through `resize()` and the `rt:` segment is reachable via `withOption('rt:...')`.
...
- Naming collisions resolved: the imgproxy `raw` processing option is `raw()` (with `withoutRaw()` for the off state; the escape hatch is `withOption()`); the server-side `preset` option is `preset()` (config-preset composition `applyPreset()` is issue 08's).
