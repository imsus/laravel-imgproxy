...
/**
 * Live check against a real imgproxy (local only).
 *
 ...
 *
 * CI does not set these variables, so the test skips there.
 */
it('serves a generated signed URL from a real imgproxy', function () {
    ...
    $signed = new Builder($url, $source, segments: ['rs:fit:300:300'], key: $key, salt: $salt);
    $unsigned = new Builder($url, $source, segments: ['rs:fit:300:300']);
